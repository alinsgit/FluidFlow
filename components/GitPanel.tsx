      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          {hasUncommittedChanges ? (
            <span className="px-1.5 py-0.5 bg-amber-500/20 text-amber-400 text-[10px] rounded flex items-center gap-1">
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0 border-r border-white/5">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            ...
            {hasUncommittedChanges && localChanges.length > 0 && (
              <div className="p-2 border-b border-amber-500/20 bg-amber-500/5">
                <div className="flex items-center justify-between px-2 py-1.5">
                  ...
                  {onDiscardChanges && (
                    <button
                      ...
                      className="flex items-center gap-1 px-2 py-0.5 text-[10px] text-red-400 hover:bg-red-500/10 rounded transition-colors"
                ...
                <div className="mt-1 space-y-0.5">
                  {localChanges.map((change) => (
                    <div
                      key={change.path}
                      className="flex items-center gap-2 px-2 py-1 ml-4 text-xs text-slate-400"
                    >
                      {change.status === 'added' && <FilePlus className="w-3 h-3 text-emerald-400" />}
                      {change.status === 'modified' && <FileText className="w-3 h-3 text-amber-400" />}
                      {change.status === 'deleted' && <FileX className="w-3 h-3 text-red-400" />}
                  ...
                  <div className="mt-2 p-2 bg-red-500/10 border border-red-500/20 rounded-lg">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1 px-2 py-1 bg-red-600 hover:bg-red-500 disabled:bg-slate-700 text-white rounded text-[10px] font-medium transition-colors"
                      >
                        {isDiscarding ? (
                      ...
                      <button
                        ...
                        className="flex-1 px-2 py-1 bg-slate-700 hover:bg-slate-600 text-white rounded text-[10px] font-medium transition-colors"
                      >
                        Cancel
                      </button>
                    </div>
                ...
                {stagedCount > 0 && (
                  <div>
                    <button
                      ...
                      className="w-full flex items-center justify-between px-2 py-1.5 hover:bg-white/5 rounded-lg transition-colors"
                    >
        ...
        <div className="w-1/2 flex flex-col min-w-0 bg-slate-950/30">
          <div className="flex items-center justify-between px-3 py-2 border-b border-white/5 flex-shrink-0">
            ...
            <span className="text-[10px] text-slate-600">{commits.length} commits</span>
          </div>
              ...
              <div className="p-2 space-y-0.5">
                {commits.map((commit, index) => (
                  <button
                    ...
                    className="w-full flex items-start gap-2 p-2 hover:bg-white/5 rounded-lg text-left transition-colors group"
                  >
                    <div className="relative flex-shrink-0">
                      <div className="w-2 h-2 rounded-full bg-blue-500 mt-1.5 group-hover:bg-blue-400 transition-colors" />
                      {index < commits.length - 1 && (
                        <div className="absolute top-3 left-1/2 -translate-x-1/2 w-px h-[calc(100%+0.25rem)] bg-slate-700" />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[11px] text-white truncate group-hover:text-blue-300 transition-colors leading-tight">
                        {commit.message}
                      </p>
  ...
  return (
    <div className="p-3">
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[11px] text-white font-medium truncate">{commit.message}</p>
          <div className="flex items-center gap-1.5 mt-0.5">
            <button
              onClick={onCopyHash}
              className="flex items-center gap-1 text-[9px] text-slate-500 font-mono hover:text-blue-400 transition-colors"
            >
            ...
            <span className="text-[9px] text-slate-600">·</span>
            <span className="text-[9px] text-slate-500">{commit.author}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mb-2 px-1">
        ...
        {commit.stats.insertions > 0 && (
          <span className="text-[10px] text-emerald-400">+{commit.stats.insertions}</span>
        )}
        {commit.stats.deletions > 0 && (
          <span className="text-[10px] text-red-400">-{commit.stats.deletions}</span>
        )}
        <button
          onClick={onViewFullDiff}
          className="ml-auto text-[9px] text-blue-400 hover:text-blue-300 flex items-center gap-0.5"
        >
      ...
      <div className="space-y-0.5">
        {commit.files.map((file) => (
          <button
            ...
            className="w-full flex items-center gap-1.5 px-2 py-1 text-[10px] text-slate-400 hover:bg-white/5 rounded group transition-colors"
          >
            ...
            <Eye className="w-2.5 h-2.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
          </button>
        ))}
      </div>
    </div>
...
const CommitFileIcon: React.FC<{ status: CommitFileChange['status'] }> = ({ status }) => {
  ...
  return <Icon className={`w-3 h-3 flex-shrink-0 ${color}`} />;
};
...
const FileItem: React.FC<FileItemProps> = ({ file, status }) => {
  ...
  return (
    <div className="flex items-center gap-2 px-2 py-1 ml-4 text-xs text-slate-400 hover:bg-white/5 rounded">
